...
time_pattern = r"(?P<time>(?P<h>\d{2}):(?P<m>\d{2}):(?P<s>\d{2})(?P<ns>.\d+)?)"
for time_match in re.finditer(time_pattern, txt):
    print()
    print('Time:', time_match)
    print('Groups:', time_match.groupdict())
    print('Hour:', time_match.group('h'))
    print('Minute:', time_match.group('m'))
    print('Second:', time_match.group('s'))
    print('Nano_secs:', time_match.group('ns'))
